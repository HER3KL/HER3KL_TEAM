...
our_id = 123456789 --Here Your Bot ID /ايدي بوتك هنا 
...
"del_msg",
    ...
    sudo_users = {},--id sudo / ايدي المطور
    admins = {}, --id admins / ايدي الادمنز
    disabled_channels = {},
    moderation = {data = './data/moderation.json'},
    info_text = [[
    TYPE CODE BOT A 2 LANG
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['banned']) == nil then --fix way
     if not lang then
					...
					return "*Group name*"..msg.to.title.."\n *لا يوجد محضورين في هذه المجموعة*"
              end
				end
       if not lang then
   message = "*Group name*"..msg.to.title.."\n *List of banned users :*\n"
         else
   message = "*Group name*"..msg.to.title.."\n_قائمه المستخدين المحضورين :_\n"
     end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['is_silent_users']) == nil then --fix way
        if not lang then
					...
					return "*Group name*"..msg.to.title.."\n*لا يوجد مستخدمين مكتومين في هذه المجموعه*"
             end
				end
      if not lang then
   message = "*Group name*"..msg.to.title.."\n*List of silent users :*\n"
       else
   message = "*Group name*"..msg.to.title.."\n _قائمة المكتومين :_\n"
    end
...
local lang = redis:get(hash)
  ...
  if next(data['gban_users']) == nil then --fix way
    if not lang then
					...
					return "*لا يوجد مستخدمين محضورين عاما*"
             end
				end
        if not lang then
   message =  "*Group name*"..msg.to.title.."\n*List of globally banned users :*\n"
   else
   message = "*Group name*"..msg.to.title.."\n _قائمة المحضورين عام :_\n"
   end
