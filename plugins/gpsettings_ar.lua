...
local lang = redis:get(hash)
  ...
  if not lang then
    return "*Group name*"..msg.to.title.."\n _No_ *moderator* _in this group_"
else
   return "*Group name*"..msg.to.title.."\n لا يوجد حاليا أي مسؤول عن المجموعة
❎ \n `MSG By:` (@"..msg.from.username or")"..msg.from.first_name
  end
end
if not lang then
...
local lang = redis:get(hash)
  ...
  if not data[tostring(msg.chat_id_)] then
if not lang then
    return "_Group is not added_ `MSG By:` (@"..msg.from.username or")"..msg.from.first_name
else
return "لم يتم اضافة البوت الى المجموعة 📛\n `MSG By:` (@"..msg.from.username or")"..msg.from.first_name
  end
end
  -- determine if table is empty
...
if not tonumber(data.sender_user_id_) then return false end
  ...
  if not lang then
    ...
    return tdcli.sendMessage(data.chat_id_, "", 0, "_لم يتم اضافة البوت الى المجموعة _", 0, "md")
     end
  end
if cmd == "تعيين مدير" then
...
local cmd = arg.cmd
  ...
  if not lang then
    ...
    return tdcli.sendMessage(data.chat_id_, "", 0, "_لم يتم اضافة البوت الى المجموعة_", 0, "md")
     end
  end
...
local settings = data[tostring(target)]["settings"] 
 text = "⚙️*اعدادات المجموعة*️\n*__________________*\n● 》_قفل التعديل ➢_ *"..settings.lock_edit.."*\n● 》_قفل الروابط ➢_ *"..settings.lock_link.."*\n● 》_قفل الكلمات السيئة ➢_ *"..settings.lock_fuck.."*\n● 》_قفل التاك ➢_ *"..settings.lock_tag.."*\n● 》_اللغة العربيه* ➢_ *"..settings.lock_arabic.."*\n● 》_قفل التكرار ➢_ *"..settings.flood.."*\n● 》_قفل السبام ➢_ *"..settings.lock_spam.."*\n● 》_قفل الاستدعاء ➢_ *"..settings.lock_mention.."*\n● 》_قفل صفحات الويب ➢_ *"..settings.lock_webpage.."*\n● 》_الترحيب ➢_ *"..settings.welcome.."*\n● 》_قفل الماركداون ➢_ *"..settings.lock_markdown.."*\n● 》_قفل البوتات ➢_ *"..settings.lock_bots.."*\n● 》_قفل الصور المتحركة ➢_ *"..settings.lock_gif.."*\n● 》_قفل النص ➢_ *"..settings.lock_text.."*\n● 》_قفل الانلاين ➢_ *"..settings.lock_inline.."*\n● 》_قفل الالعاب ➢_ *"..settings.lock_game.."*\n● 》_قفل الصور ➢_ *"..settings.lock_photo.."*\n● 》_قفل الفيديو ➢_ *"..settings.lock_video.."*\n● 》_قفل الصوتيات ➢_ *"..settings.lock_audio.."*\n● 》_قفل الصوت ➢_ *"..settings.lock_voice.."*\n● 》_قفل الملصقات ➢_ *"..settings.lock_sticker.."*\n● 》_قفل جهات الاتصال ➢_ *"..settings.lock_contact.."*\n● 》_قفل اعاده التوجيه ➢_ *"..settings.lock_forward.."*\n● 》_قفل الموقع ➢_ *"..settings.lock_location.."*\n● 》_قفل الملفات ➢_ *"..settings.lock_document.."*\n● 》_قفل خدمه TG ➢_ *"..settings.lock_tgservice.."*\n● 》_قفل الكل : _ *"..settings.lock_all.."*\n*__________________*\n● 》_عدد التكرار ➢_ *"..NUM_MSG_MAX.."*\n● 》_وقت التصفح ➢_ *"..expire.."*\n*__________________*\n مفتوح✓ ➰ مغلق✘ \n*__________________*\n `support:` @lock_at_me_now or @THS_56 \n*__________________*\n*Language* : *ar*"
else
local settings = data[tostring(target)]["settings"] 
...
if matches[1] == "الغاء" then
return modrem(msg)
end
if matches[1] == "تعيين مدير" and is_admin(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
      message_id_ = msg.reply_to_message_id_
    }, action_by_reply, {chat_id=msg.chat_id_,cmd="تعيين مدير"})
  end
  if matches[2] and string.match(matches[2], '^%d+$') then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  }, action_by_id, {chat_id=msg.chat_id_,user_id=matches[2],cmd="تعيين مدير"})
    end
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
      ID = "SearchPublicChat",
      username_ = matches[2]
    }, action_by_username, {chat_id=msg.chat_id_,username=matches[2],cmd="تعيين مدير"})
      end
   end
if matches[1] == "ازالة مدير" and is_admin(msg) then
  if matches[2] and string.match(matches[2], '^%d+$') then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
...
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ...
      chat_id_ = msg.chat_id_,
      message_id_ = msg.reply_to_message_id_
    }, action_by_reply, {chat_id=msg.chat_id_,cmd="ازالة مدير"})
  end
  if matches[2] and string.match(matches[2], '^%d+$') then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  }, action_by_id, {chat_id=msg.chat_id_,user_id=matches[2],cmd="ازالة مدير"})
    end
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
      ID = "SearchPublicChat",
      username_ = matches[2]
    }, action_by_username, {chat_id=msg.chat_id_,username=matches[2],cmd="ازالة مدير"})
      end
   end
...
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ...
      chat_id_ = msg.chat_id_,
      message_id_ = msg.reply_to_message_id_
    }, action_by_reply, {chat_id=msg.chat_id_,cmd="ترقية"})
  end
  if matches[2] and string.match(matches[2], '^%d+$') then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  }, action_by_id, {chat_id=msg.chat_id_,user_id=matches[2],cmd="ترقية"})
    end
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
      ID = "SearchPublicChat",
      username_ = matches[2]
    }, action_by_username, {chat_id=msg.chat_id_,username=matches[2],cmd="ترقية"})
      end
   end
if matches[1] == "خفض" and is_owner(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
 tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
      message_id_ = msg.reply_to_message_id_
    }, action_by_reply, {chat_id=msg.chat_id_,cmd="خفض"})
  end
  if matches[2] and string.match(matches[2], '^%d+$') then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  }, action_by_id, {chat_id=msg.chat_id_,user_id=matches[2],cmd="خفض"})
    end
  if matches[2] and not string.match(matches[2], '^%d+$') then
    tdcli_function ({
      ID = "SearchPublicChat",
      username_ = matches[2]
    }, action_by_username, {chat_id=msg.chat_id_,username=matches[2],cmd="خفض"})
      end
   end

if matches[1] == "قفل" and is_mod(msg) then
local target = msg.chat_id_
if matches[2] == "الروبط" then
...
if matches[2] == "اعادة التوجيه" then
...
if matches[1] == "معلومات المجموعة" and is_mod(msg) and gp_type(msg.chat_id_) == "channel" then
...
print(serpent.block(data))
 ...
 tdcli.getChannelFull(msg.chat_id_, group_info, {chat_id=msg.chat_id_,msg_id=msg.id_})
		...
		if matches[1] == 'تعيين رابط' and is_owner(msg) then
   ...
   local is_link = msg.content_.text_:match("^([https?://w]*.?telegram.me/joinchat/%S+)$") or msg.content_.text_:match("^([https?://w]*.?t.me/joinchat/%S+)$")
			if is_link and data[tostring(chat)]['settings']['linkgp'] == 'الرجاء الانتضار...' and is_owner(msg) then
  ...
  if matches[1] == "تعيين قوانين" and matches[2] and is_mod(msg) then
...
if matches[1] == "معلومات" and matches[2] and is_mod(msg) then
    tdcli_function ({
      ID = "SearchPublicChat",
      username_ = matches[2]
    }, action_by_username, {chat_id=msg.chat_id_,username=matches[2],cmd="معلومات"})
  end
if matches[1] == "من هوه" and matches[2] and is_mod(msg) then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  }, action_by_id, {chat_id=msg.chat_id_,user_id=matches[2],cmd="من هوه"})
  end
  if matches[1] == 'تعيين تكرار' and is_mod(msg) then
    ...
    return "_Group_ *flood* _sensitivity has been set to :_ *[ "..matches[2].." ]*"
		...
		if matches[1]:lower() == 'تنضيف' and is_owner(msg) then
			...
			if matches[2] == 'قائمه الكلمات' then
     ...
     if not lang then
       ...
       if not lang then
            ...
            return "لقد تم ازاله رساله الترحيب ❌ \n `MSG BY:` *"..msg.from.first_name..'*'
			end
       end
   ...
   tdcli.changeChannelAbout(chat, "", dl_cb, nil)
		...
		if matches[1]:lower() == 'تنضيف' and is_admin(msg) then
...
if matches[1] == " تعيين اسم" and matches[2] and is_mod(msg) then
local gp_name = matches[2]
tdcli.changeChatTitle(chat, gp_name, dl_cb, nil)
end
  if matches[1] == " تعيين وصف" and matches[2] and is_mod(msg) then
 ...
 if not data[tostring(chat)]['about'] then
  ...
  if matches[1] == 'كلمه' and is_mod(msg) then
    return filter_word(msg, matches[2])
  end
  if matches[1] == 'ازاله كلمه' and is_mod(msg) then
...
if matches[1] == "قائمة القفل" then
return locks(msg, target)
end
if matches[1] == "قائمة المشرفين" then
return modlist(msg)
end
if matches[1] == "قائمة المدراء" and is_owner(msg) then
...
if matches[1] == "مساعده" and is_mod(msg) then
if not lang then
text = [[
...
if matches[1] == "انلاين" and is_mod(msg) then
text5 = [[
inline💡
]]
return text5 
...
--------------------- Welcome -----------------------
	...
	if matches[1] == "تعيين ترحيب" and matches[2] and is_mod(msg) then
